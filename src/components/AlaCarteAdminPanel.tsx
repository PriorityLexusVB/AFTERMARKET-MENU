...
interface AlaCarteAdminPanelProps {
  onDataUpdate: () => void;
  onEditItem?: (optionId: string) => void;
}
...
const SortableAlaCarteItem: React.FC<SortableAlaCarteItemProps> = ({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`bg-gray-800 p-3 rounded-md border ${
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2 flex-1">
          ...
          <button
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing text-gray-500 hover:text-gray-300 p-1 -ml-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/80 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-900 rounded"
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onMoveUp}
            disabled={isFirst}
            className="text-gray-500 hover:text-gray-300 disabled:opacity-30 disabled:cursor-not-allowed p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/80 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-900 rounded"
            title="Move up"
          ...
          {onEdit && (
            <button
              ...
              className="text-blue-400 hover:text-blue-300 transition-colors text-xs underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/80 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-900 rounded"
            >
              Edit in Product Hub
            </button>
          )}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`text-xs font-semibold px-1.5 py-0.5 rounded cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/80 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-900 ${
            connector === "OR"
...
export const AlaCarteAdminPanel: React.FC<AlaCarteAdminPanelProps> = ({
  onDataUpdate,
  onEditItem,
}) => {
  ...
  const fetchOptions = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Failed to fetch A La Carte options. Please check your Firestore rules and connection."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const applyLaneUpdates = async (
    nextFeatured: AlaCarteOption[],
    nextPublished: AlaCarteOption[]
  ) => {
            ...
            <div className="space-y-2">
              {columnOptions.map((option, index) => (
                <SortableAlaCarteItem
                  ...
                  onEdit={onEditItem ? () => onEditItem(option.id) : undefined}
                  isFirst={index === 0}
              ...
              {options.length === 0 ? (
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div
                      className="bg-gray-900/30 p-4 rounded-lg border border-gray-700 relative max-h-[70vh] overflow-y-auto"
                      data-testid="column-published"
                    >
                      <div className="sticky top-0 z-10 bg-gray-900/90 backdrop-blur-sm pb-2">
                        <h5 className="text-lg font-semibold text-gray-300 mb-1 font-teko tracking-wider">
                          Published (Not featured)
                        </h5>
                        <p className="text-xs uppercase tracking-[0.2em] text-lux-textMuted">
                          Available, but not in featured
                        </p>
                      </div>
                      <div className="mt-2">
