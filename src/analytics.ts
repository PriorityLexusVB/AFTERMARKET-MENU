...
export function trackAIAssistantOpen(): void {
  ...
  logEvent(analytics, 'ai_assistant_open');
}

/**
 * Track AI Assistant message sent
 */
export function trackAIAssistantMessage(messageLength: number): void {
  if (!analytics) return;

  logEvent(analytics, 'ai_assistant_message', {
    message_length: messageLength,
  });
}

/**
 * Track AI Assistant close
 */
export function trackAIAssistantClose(messageCount: number): void {
  if (!analytics) return;

  logEvent(analytics, 'ai_assistant_close', {
