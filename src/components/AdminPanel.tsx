...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onDataUpdate }) => {
  ...
  const fetchFeatures = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Failed to fetch product features. Please check your Firestore rules and connection."
      );
    } finally {
      setIsLoading(false);
    }
        ...
        (acc: { published: number; featured: number }, doc: any) => {
          const data =
            typeof doc.data === "function"
              ? (doc.data() as { isPublished?: boolean; column?: number })
              : undefined;
  ...
  const handleEditInProductHub = useCallback(
    (featureId: string) => {
  ...
  const handleRecommendedChange = useCallback(
    async (packageId: string | "none") => {
  ...
  const findColumnForFeature = useCallback(
    (featureId: string): number | "unassigned" | null => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (targetColumn !== null) {
      if (targetColumn !== activeColumn) {
        ...
        const targetColumnFeatures =
          targetColumn === "unassigned"
  ...
  const renderColumnFeatures = (
    columnFeatures: ProductFeature[],
    columnId: number | "unassigned",
  ...
  return (
    <main className="container mx-auto px-4 py-4 md:px-6 md:py-6 max-w-screen-2xl flex-grow flex flex-col">
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 md:p-8 flex-grow">
        ...
        {activeTab === "alacarte" ? (
          <AlaCarteAdminPanel
            onDataUpdate={handleAlaCarteDataUpdate}
            onEditItem={handleEditInProductHub}
          />
        ) : activeTab === "product-hub" ? (
          <ProductHub
