...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onDataUpdate }) => {
  ...
  const [showStrictMappingWarning, setShowStrictMappingWarning] = useState(false);
  ...
  // Detect empty package columns (1, 2, 3) - used for strict mapping warning
  const emptyPackageColumns = useMemo(() => {
    const empty: number[] = [];
    if (featuresByColumn[1].length === 0) empty.push(1);
    if (featuresByColumn[2].length === 0) empty.push(2);
    if (featuresByColumn[3].length === 0) empty.push(3);
    return empty;
  }, [featuresByColumn]);

  // Show strict mapping warning when any package column (1, 2, 3) is empty
  useEffect(() => {
    setShowStrictMappingWarning(emptyPackageColumns.length > 0);
  }, [emptyPackageColumns]);
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 md:p-8 flex-grow">
        ...
        {activeTab === 'alacarte' ? (
          ...
          <>
            ...
            {/* Strict Mapping Warning Banner */}
            {showStrictMappingWarning && (
              <div className="mb-6 bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4" data-testid="strict-mapping-warning">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="text-yellow-300 font-semibold mb-2">⚠️ Empty Package Column{emptyPackageColumns.length > 1 ? 's' : ''} Detected</h4>
                    <p className="text-yellow-300/90 text-sm mb-2">
                      Packages are now <strong>strictly mapped</strong> to columns: <strong>Gold = Column 1</strong>, <strong>Elite = Column 2</strong>, <strong>Platinum = Column 3</strong>.
                    </p>
                    <p className="text-yellow-300/90 text-sm mb-2">
                      The following column{emptyPackageColumns.length > 1 ? 's are' : ' is'} currently empty: <strong>{emptyPackageColumns.map(col => {
                        const labels = { 1: 'Column 1 (Gold)', 2: 'Column 2 (Elite)', 3: 'Column 3 (Platinum)' };
                        return labels[col as keyof typeof labels];
                      }).join(', ')}</strong>
                    </p>
                    <p className="text-yellow-300/90 text-sm mb-3">
                      When the <code className="bg-yellow-900/30 px-1.5 py-0.5 rounded text-xs">VITE_ALLOW_PACKAGE_FEATUREIDS_FALLBACK</code> environment variable is disabled (set to <code className="bg-yellow-900/30 px-1.5 py-0.5 rounded text-xs">false</code>), 
                      packages with empty columns will have <strong>no features</strong> on the customer-facing UI.
                    </p>
                    <div className="bg-yellow-900/20 border border-yellow-500/20 rounded p-3 text-sm">
                      <p className="text-yellow-200 font-semibold mb-1">🔧 How to Fix:</p>
                      <ol className="text-yellow-300/90 list-decimal list-inside space-y-1">
                        <li>Go to <button onClick={() => handleTabChange('product-hub')} className="underline hover:text-yellow-200 font-semibold">Product Hub</button> to edit features</li>
                        <li>Assign each feature to the correct column for its tier</li>
                        <li>If a feature should appear in multiple tiers, duplicate it and assign copies to different columns</li>
                        <li>Once all tiers have features assigned, you can safely disable the fallback</li>
                      </ol>
                    </div>
                  </div>
                </div>
